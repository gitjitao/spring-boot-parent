...
import com.springboot.util.ObjectUtils;
...
public class JedisService {
	...
	public void expire(String key, int cacheSeconds) {
		...
		try {
			...
			logger.debug("expire {} = {} s", key, cacheSeconds);
		} catch (Exception e) {
			logger.warn("expire {} = {} s", key, cacheSeconds , e);
		} finally {
			returnResource(jedis);
		}
	}
	...
	public Object hget(String key,String hkey) {
		...
		try {
			...
			value = toObject(jedis.hget(getBytesKey(key), getBytesKey(hkey)));
		} catch (Exception e) {
			logger.warn("delObject {}", key, e);
		} finally {
			returnResource(jedis);
	...
	public void hset(String key,String hkey,Object obj) {
		Jedis jedis = null;
		try {
			jedis = getResource();
			jedis.hset(getBytesKey(key), getBytesKey(hkey), toBytes(obj));
		} catch (Exception e) {
			logger.warn("delObject {}", key, e);
		} finally {
			returnResource(jedis);
	...
	public Long llrem(final String key, final long count, final String value) {
		Jedis jedis = null;
		try {
			jedis = getResource();
